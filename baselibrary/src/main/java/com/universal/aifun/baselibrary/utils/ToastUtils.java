...
public class ToastUtils {
    ...
    public static void showShortToastFromRes(final @StringRes int resId) {
        showShortToastFromText(TKBaseApplication.myApplication.getResources().getString(resId), ToastUtils.bottom);
    }
    ...
    private static void showToast(CharSequence text, int duration, int locationflag) {
        try {
            cancelToast();
            if (locationflag == center) {
                if (sToast == null) {
                    View view = LayoutInflater.from(TKBaseApplication.myApplication).inflate(R.layout.toast_view, null);
